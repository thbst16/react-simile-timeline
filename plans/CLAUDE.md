...
| [v1.1.0 toolchain modernization](2026-08-18-toolchain-modernization-plan.md)                   | Milestone `v1.1.0 - Toolchain modernization` (#18–#24, #33, #36, #37, #42, #44). Every dependency major current and all 42 security findings cleared — 40 Dependabot advisories and 2 CodeQL alerts — ESLint on flat config, coverage measured for the first time, unit tests for the two untested core files, the React 19 peer promise proven or narrowed, the unverified WCAG claim corrected, and a token/version-guarded release with npm provenance. **Complete 2026-08-19, released as `1.1.0`.** OIDC trusted publishing prepped (#63) and three dev-only advisories carried to v1.1.1. Excludes #25, the WCAG audit itself.                                                                                                                                                                    |
| [v1.1.1 security & release hardening](2026-08-19-security-release-hardening-plan.md)           | The toolchain and release follow-ups left after `v1.1.0`: the three dev-only advisories (drop `vite-plugin-dts` for a direct api-extractor rollup), the OIDC publish cutover that removes `NPM_TOKEN` (#42/#63), the `brace-expansion`/`minimatch` hazard (#61), and enforcing the three new CI checks. Publishes `1.1.1`. Completes the actionable half; defers #50/#54/#56/#25 with explicit unblock triggers. **Complete 2026-08-20, released as `1.1.1` and attested.** §1.1 cleared the high-severity `rollup` advisory and `vue-template-compiler` (#52/#66); §3 closed #61 (#67); §4 took required checks to 10. Advisories 3 → 1 (`esbuild`, an unmovable dev-only Vite optional peer, recorded on #52). Shipped on the token path; OIDC (#63) carried forward as a draft for a future release. |
| [v1.2.0 accessibility & final hardening](2026-08-20-accessibility-and-final-hardening-plan.md) | Every remaining issue worth doing, in one release: the WCAG 2.1 AA audit and remediation (#25, the substantive core — ARIA today in only 3 of 9 components, no axe gate, no reduced-motion), the OIDC publish cutover (#42/#63), ESLint 9 → 10 via the `@eslint-react` plugin swap (#54, which dissolves #61's root), and the `esbuild` advisory via a pnpm 9 → 11 upgrade (#52). Publishes `1.2.0`. Out with evidence: #56 (TypeScript 7 — no upstream support in any tag) and #50 (jsdom 30 — net-negative, recommend close). Features #1–#5 closed as not-planned.                                                                                                                                                                                                                                   |
